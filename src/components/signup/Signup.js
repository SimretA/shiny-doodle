...
import {emailValidation, passwordValidation, phoneValidation} from "../../control/validation";
...
export function Signup() {
    ...
    const handleSignup = (evt) => {
        ...
        else if (!emailValidation(account.email)) {
            setWarn("Invalid Email");
            return
        }
        else if (!passwordValidation(account.password)) {
            setWarn("Password not strong enough");
            return
        }
        else if(!match){
        ...
        <Wrapper>
            ...
            <FormContainer>
                <Column>
                    ...
                    <InputContainer>
                        <Label htmlFor="inputPassword1">Confirm</Label>
                        <TextInput type="password"  id="confirmPass" placeholder="Confirm Password"
                                   style={{borderColor: !match?"red":""}}
                                   onChange={evt => {
                                       matchPassword(evt.target.value);
                                   }}/>
